...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && band.current) {
      ...
      if (geometry && typeof geometry.setPoints === 'function') {
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            ...
            onPointerDown={(e: React.PointerEvent<HTMLDivElement>) => {
              ...
              const point = new THREE.Vector3(e.clientX, e.clientY, 0);
              drag(point.sub(vec.copy(card.current.translation())));
            }}>
            <mesh geometry={(nodes.card as THREE.Mesh).geometry}>
              <meshPhysicalMaterial map={(materials.base as THREE.MeshStandardMaterial).map} map-anisotropy={16} clearcoat={1} clearcoatRoughness={0.15} roughness={0.9} metalness={0.8} />
            </mesh>
